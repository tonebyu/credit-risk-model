...
def assign_high_risk_labels(input_csv_path: str, output_csv_path: str, snapshot_date: str = "2023-01-01"):
    df = pd.read_csv(input_csv_path)
     ...
     # Ensure consistent timezone-naive datetime
    df['TransactionStartTime'] = pd.to_datetime(df['TransactionStartTime'], errors='coerce').dt.tz_localize(None)
    snapshot_date = pd.to_datetime(snapshot_date).tz_localize(None)

    
    df['TransactionStartTime'] = pd.to_datetime(df['TransactionStartTime'], errors='coerce')
    rfm_df = df[['CustomerId', 'TransactionStartTime', 'total_amount']].copy()
